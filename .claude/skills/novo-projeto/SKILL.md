...
- **Cliente novo:** criar em `clientes/<Nome>/` — só cliente de verdade vai
  aqui (ler `CLAUDE.md` da raiz pra confirmar a convenção)
- **Projeto interno ou iniciativa pessoal:** criar em `meus-projetos/<nome>/`
  — é a pasta dos projetos do próprio usuário, separada de `clientes/`
  ("eu não sou meu cliente"). Não perguntar onde salvar: a convenção já
  existe, só confirmar o nome da pasta
...
Esse projeto herda automaticamente o tom de voz, marca e contexto do negócio definidos em `obsidian-vault/` e `identidade/` da raiz. Não duplicar essas informações aqui.

Se o projeto envolver site, herda também as regras técnicas fixas do
PedrosOS (mobile-first, proteção de texto, zoom, hero, Google Ads,
rastreador, git por projeto, deploy, segurança) — texto completo no
`CLAUDE.md` da raiz do workspace, seção "Regras do sistema". Ler antes de
qualquer tarefa de site nesse projeto.

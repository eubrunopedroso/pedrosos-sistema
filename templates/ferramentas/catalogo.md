...
### `scripts/gerar-imagem.js` — ✅ ativo neste projeto
**O que faz:** Gera imagens a partir de texto, com fallback automático em
cadeia — tenta Gemini (Nano Banana 2, mais barato) primeiro, cai pro
OpenAI (gpt-image-1, melhor qualidade) se faltar crédito/chave, e cai
pro Pollinations.ai/FLUX (grátis, sem chave nenhuma) se os dois
anteriores falharem. Já é o único módulo que fala com provedor de
imagem — nenhuma skill deve chamar API de imagem direto
**Precisa de conta:** Não — funciona sem nenhuma chave configurada
(usa o fallback grátis). Com `GEMINI_API_KEY` e/ou `OPENAI_API_KEY` no
`.env`, sobe de qualidade automaticamente
**Configurar:** Opcional — `GEMINI_API_KEY` (Google AI Studio) e/ou
`OPENAI_API_KEY` (OpenAI, pago por imagem) no `.env`
**Quando usar:** Padrão do projeto pra qualquer foto IA — capas, ilustrações, imagens pra posts

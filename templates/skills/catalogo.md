...
**Como instalar:** Nao vem instalada por padrao neste pacote — se quiser essa metodologia especifica, peca pro Claude criar via `/mapear-rotinas`, usando esse resumo como briefing
**Fonte:** Referencia de metodologia, nao skill pronta
...
**Precisa de:** yt-dlp instalado (`pip install yt-dlp` ou `brew install yt-dlp` no Mac)
**Como instalar:** Nao vem instalada por padrao — o comando `yt-dlp --write-auto-subs --sub-format vtt --skip-download` ja traz a transcricao direto, sem precisar de skill nenhuma
**Fonte:** Referencia de ferramenta, nao skill pronta
